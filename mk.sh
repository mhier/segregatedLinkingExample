...
# original commands to show how segregated linking works:
...
# commands to show the segfault problem:
g++ -g -O0 -std=c++17 -fPIC -shared libC.cc -o libC.so -L. `pkg-config libxml++-2.6 --libs --cflags`
g++ -g -O0 -std=c++17 main.cc -o main -L. -ldl -Wl,-rpath=. `pkg-config libxml++-2.6 --libs --cflags` -DWITH_CRASH
